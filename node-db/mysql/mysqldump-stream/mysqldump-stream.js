...
function Mysqldump(database, options) {
	...
	var opts = {
		...
		mysqlBinDir: options.mysqlBinDir || '',  // end with '/'
		configPath: options.configPath  // path of config.cnf

	}
	this.dump = this.start = this.run = this.make = function () {
		var params = ['--defaults-extra-file=' + opts.configPath, '-h', opts.host, '-P', opts.port /* , '-u', opts.user */];
		var gzip;

		// user, password options have been replaced with --defaults-extra-file
		/*
		...
		*/
		...
		// console.log('spawn: ', opts.mysqlBinDir + 'mysqldump', params);

		this.mysqldump = spawn(opts.mysqlBinDir + 'mysqldump', params);
